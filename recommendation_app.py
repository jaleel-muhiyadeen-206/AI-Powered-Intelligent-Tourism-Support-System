...
from recommendation_model.models.model_1_you_may_also_like import Model1_YouMayAlsoLike
from recommendation_model.models.model_2_popular_nearby import Model2_PopularNearby
from recommendation_model.models.model_3_nearby_essentials import Model3_NearbyEssentials
...
USER_LIKES_PATH = 'recommendation_model/data/user_likes.csv'
USER_REVIEWS_PATH = 'recommendation_model/data/user_reviews.csv'
USER_RATINGS_PATH = 'recommendation_model/data/user_ratings.csv'

# ── Image URL Overrides (UI-only, no retraining needed) ──────────────
# Add place names here to override their image URLs at display time.
IMAGE_URL_OVERRIDES = {
    'Batadombalena (Prehistoric Cave)': 'https://upload.wikimedia.org/wikipedia/commons/8/8a/Batadombalena_archaeological_sites.JPG',
    'Diva Guhava (Batatotalena)': 'https://media-cdn.tripadvisor.com/media/photo-s/1c/c1/53/f8/the-batatotalena-cave.jpg',
    'Khan Clock Tower': 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/0e/ac/68/56/khan-clock-tower.jpg?w=1200&h=-1&s=1',
}
...
@st.cache_resource
def load_models():
    sub1 = joblib.load('recommendation_model/output/preprocessed/submodel_1.joblib')
    model1 = Model1_YouMayAlsoLike(sub1)
    model1.load('recommendation_model/output/trained_models/model_1.joblib')
    model2 = Model2_PopularNearby(pd.DataFrame())
    model2.load('recommendation_model/output/trained_models/model_2.joblib')
    model3 = Model3_NearbyEssentials(pd.DataFrame(), pd.DataFrame())
    model3.load('recommendation_model/output/trained_models/model_3.joblib')
...
def render_rec_card_html(rank, name, rating, distance, review, place_type="",
    ...
    fallback_div = ('<div class="rec-card-placeholder" style="background:linear-gradient(135deg,#7c3aed 0%,#a855f7 100%);'
    ...
    if _is_valid_image_url(image_url):
        ...
        placeholder = ('<div class="rec-card-placeholder" style="display:none;background:linear-gradient(135deg,#7c3aed 0%,#a855f7 100%);'
                       f'color:rgba(255,255,255,0.85);font-size:1.5rem;font-weight:700;">{initials}</div>')
    else:
        img_html = ''
    ...
    card_html = (
        ...
        f'<div class="rec-card-img-wrap">'
        f'{img_html}{placeholder}'
        f'<div class="rec-card-badge">#{rank}</div>'
        f'<div class="rec-card-distance">{distance} km</div>'
        f'</div>'
        f'<div class="rec-card-body">'
        f'<div class="rec-card-name">{safe_name}</div>'
        f'<div class="rec-card-meta">'
        f'<span class="rec-card-rating">★ {rating}/5</span>'
        f'{type_html}'
        f'</div>'
        f'{district_html}'
        f'<div class="rec-card-review">"{safe_review}"</div>'
        f'</div>'
        f'</div>'
    )
    return card_html
    ...
    for i, (_, rec) in enumerate(recs_df.iterrows()):
        ...
        with cols[i]:
            html = render_rec_card_html(
                ...
                rec.get('district', ''),
                IMAGE_URL_OVERRIDES.get(rec['name'], rec.get('image', ''))
            )
...
def render_service_card(rank, name, rating, distance, review, budget="", image_url=""):
    ...
    initials = ''.join(w[0] for w in str(name).split()[:2] if w).upper() or ''
    fallback_svc = ('<div style="width:120px;min-height:120px;background:linear-gradient(135deg,#7c3aed 0%,#a855f7 100%);'
                    'display:flex;align-items:center;justify-content:center;'
                    f'color:rgba(255,255,255,0.85);font-size:1.2rem;font-weight:700;flex-shrink:0;">{initials}</div>')
    if _is_valid_image_url(image_url):
        onerror = "this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\';"
        ...
        img_html = fallback_svc
    budget_html = f'<div class="service-card-budget">💰 {safe_budget}</div>' if safe_budget else ''
    card_html = (
        f'<div class="service-card">'
        f'{img_html}'
        f'<div class="service-card-body">'
        f'<div class="service-card-name"><span class="rank-badge">#{rank}</span>{safe_name}</div>'
        f'<div class="service-card-meta">'
        f'<span class="service-card-rating">★ {rating}/5</span>'
        f'<span class="service-card-distance">{distance} km</span>'
        f'</div>'
        f'{budget_html}'
        f'<div class="service-card-review">"{safe_review}"</div>'
        f'</div>'
        f'</div>'
    )
    return card_html
...
def render_place_results(place, sub1, model1, model2, model3):
    ...
    image_url = IMAGE_URL_OVERRIDES.get(pname, place.get('image_url', ''))
